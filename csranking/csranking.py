...
def disambiguation(filepath1,filepath2):
    ...
    if len(check)!=len(author):
        print("Some urls of authors' DBLP entry are missing. You need check manually and delete some urls in author_modified1.csv.")
        return
...
def duplicate(filepath):
    ...
    for i in range(0,l):
        ...
        if name not in author_name:
            ...
            author_affiliation.append(affiliation)
    f2 = open('author.csv','w',encoding='utf-8')
    csv_writer = csv.writer(f2)
    num=len(author_name)
    for i in range(0,num):
...
if __name__ == '__main__':
    ...
    # get_urls(f1)
    f2 = 'paper_url.csv'
    get_affiliations_tcss(f2)
    """
    get_unique(f1)

    # check manually

    disambiguation(f1,'author_modified1.csv')

    normalize('country-info.csv','author_modified.csv')

    # check manually

    duplicate('author_modified.csv')
    """